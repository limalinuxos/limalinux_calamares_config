...
def move_mkinitcpio_preset():
    target_root = libcalamares.globalstorage.value("rootMountPoint")
    src = os.path.join(target_root, "etc/mkinitcpio.d/limalinux")
    dst = os.path.join(target_root, "etc/mkinitcpio.d/linux.preset")

    libcalamares.utils.debug("-> Moving limalinux preset to linux.preset in target...")
    try:
        os.replace(src, dst)
